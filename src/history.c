...
HISTORY_STATE *history_get_history_state (__history) void *__history;
{
...
void history_set_history_state (__history, state) void *__history;
HISTORY_STATE *state;
{
    struct history *history = (struct history *)__history;
...
void using_history (__history) void *__history;
{
    struct history *history = (struct history *)__history;
...
int history_total_bytes (__history) void *__history;
{
    struct history *history = (struct history *)__history;
...
int where_history (__history) void *__history;
{
    struct history *history = (struct history *)__history;
...
int history_set_pos (__history, pos) void *__history;
int pos;
{
    struct history *history = (struct history *)__history;
...
HIST_ENTRY **history_list (__history) void *__history;
{
    struct history *history = (struct history *)__history;
...
HIST_ENTRY *current_history (__history) void *__history;
{
    struct history *history = (struct history *)__history;

    return ((history->history_offset == history->history_length)
            || history->the_history == 0)
               ? (HIST_ENTRY *)NULL
...
/* Back up history_offset to the previous history entry, and return
   ...
   a NULL pointer. */
HIST_ENTRY *previous_history (__history) void *__history;
{
    struct history *history = (struct history *)__history;
...
/* Move history_offset forward to the next history entry, and return
   ...
   NULL pointer. */
HIST_ENTRY *next_history (__history) void *__history;
{
    struct history *history = (struct history *)__history;
...
HIST_ENTRY *history_get (__history, offset) void *__history;
int offset;
{
    struct history *history = (struct history *)__history;
    int local_index;
    ...
    return (local_index >= history->history_length || local_index < 0
            || !history->the_history)
               ? (HIST_ENTRY *)NULL
...
/* Place STRING at the end of the history list.  The data field
   is  set to NULL. */
void add_history (__history, string) void *__history;
char *string;
{
    struct history *history = (struct history *)__history;
    HIST_ENTRY *temp;
    ...
    if (history->history_stifled
        && (history->history_length == history->max_input_history)) {
        register int i;
	 ...
	 and it equals max_input_history, we don't save items. */
        ...
        if (history->history_size == 0) {
            ...
            history->the_history = (HIST_ENTRY **)g_malloc (
                history->history_size * sizeof (HIST_ENTRY *));
            history->history_length = 1;
        } else {
            if (history->history_length == (history->history_size - 1)) {
                history->history_size += DEFAULT_HISTORY_GROW_SIZE;
                history->the_history = (HIST_ENTRY **)g_realloc (
                    history->the_history,
                    history->history_size * sizeof (HIST_ENTRY *));
            }
...
HIST_ENTRY *replace_history_entry (__history, which, line,
                                   data) void *__history;
int which;
char *line;
histdata_t data;
...
HIST_ENTRY *remove_history (__history, which) void *__history;
int which;
{
    struct history *history = (struct history *)__history;
...
void stifle_history (__history, max) void *__history;
int max;
{
    struct history *history = (struct history *)__history;
...
int unstifle_history (__history) void *__history;
{
    struct history *history = (struct history *)__history;
...
int history_is_stifled (__history) void *__history;
{
    struct history *history = (struct history *)__history;
...
void clear_history (__history) void *__history;
{
    struct history *history = (struct history *)__history;
    register int i;
