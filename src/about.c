...
create_about_window (GtkWidget *widget, gpointer data)
{
...

    {
        ...
        if (parent) {
            gtk_window_set_transient_for (GTK_WINDOW (about_window), parent);
        }
    }

    g_signal_connect (about_window, "destroy", G_CALLBACK (on_about_destroy),
                      NULL);
    ...
    markup = g_strdup_printf (
        "<span size=\"x-large\" weight=\"bold\">GtkHx %s</span>", VERSION);
