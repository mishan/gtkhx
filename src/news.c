...
news_search_ensure_tags (GtkTextBuffer *buf)
{
	...
	if (!gtk_text_tag_table_lookup (tt, "search-match")) {
		gtk_text_buffer_create_tag (buf, "search-match",
		                            "background", "#f6d32d",
		                            "foreground", "#000000",
		                            NULL);
	}
	if (!gtk_text_tag_table_lookup (tt, "search-current")) {
		gtk_text_buffer_create_tag (buf, "search-current",
		                            "background", "#ff7800",
		                            "foreground", "#ffffff",
		                            NULL);
	}
...
{
	GtkTextBuffer *buf = gtk_text_view_get_buffer (ctx->text_view);
	const char *needle = gtk_editable_get_text (GTK_EDITABLE (ctx->search_entry));
	...
	while (TRUE) {
		...
		gtk_text_buffer_apply_tag_by_name (buf, "search-match",
		                                   &ms, &me);
		cursor = me;
	}
...
on_find_btn_clicked (GtkButton *btn, gpointer data)
{
	...
	gboolean was_open;
	(void) btn;
	was_open = gtk_search_bar_get_search_mode (GTK_SEARCH_BAR (ctx->search_bar));
...
news_search_bar_new (struct news_search_ctx *ctx)
{
	...
	gtk_search_bar_connect_entry (GTK_SEARCH_BAR (bar),
	                              GTK_EDITABLE (entry));
	...
	g_signal_connect (entry, "next-match",
	                  G_CALLBACK (on_search_next),     ctx);
	g_signal_connect (entry, "previous-match",
	                  G_CALLBACK (on_search_prev),     ctx);
	g_signal_connect (entry, "activate",
	                  G_CALLBACK (on_search_next),     ctx);
	g_signal_connect (prev_btn, "clicked",
	                  G_CALLBACK (on_search_btn_prev), ctx);
	g_signal_connect (next_btn, "clicked",
	                  G_CALLBACK (on_search_btn_next), ctx);
...
void create_news_window (session *sess)
{
	...
	search_ctx->matches       = g_array_new (FALSE, FALSE,
	                                         sizeof (struct news_match));
...
void output_news_post (struct htlc_conn *htlc, char *news, guint16 len)
{
	 ...
	 * entry is empty. */
	{
		struct news_search_ctx *sctx = g_object_get_data (
			G_OBJECT (sess->news_window), "search-ctx");
		if (sctx)
