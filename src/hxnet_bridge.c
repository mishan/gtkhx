...
hx_bridge_dispatch_shutdown (struct htlc_conn *htlc, int reason)
{
     ...
     * send-path failure path called close first, but hxnet had
     * already forwarded an `Event::Shutdown` onto the idle queue).
     * Re-entering `hx_htlc_close` would run its teardown a second
     * time.
     *
     * The re-entry guard is `htlc->fd == 0` ONLY: hx_htlc_close sets
     * fd to 0, so that flag alone distinguishes "already closed". We
     * must NOT also gate on `!hx_bridge_is_installed()` here the way
     * hx_bridge_dispatch_frame does — bridge_on_shutdown_cb clears
     * `bridge_handle` before dispatching, so the bridge always reads
     * as uninstalled at this point, and that term would wrongly
     * suppress hx_htlc_close on every normal actor shutdown (EOF /
     * StreamError), leaving the connection wedged. */
    if (htlc->fd == 0) {
        return;
    }
