                             ...
                             gpointer user_data, GDestroyNotify user_data_free)
{
    ...
    int32_t hc = gtkhx_proto_build_upload_media_single_chunks (
        payload, payload_len, (const uint8_t *)declared_type, declared_type_len,
        chunks, G_N_ELEMENTS (chunks), scratch, sizeof (scratch));
    if (hc <= 0) {
        debug_log ("media",
                   "upload: single-shot builder rejected payload "
    ...
    struct task *tsk = task_new (htlc, RCV_TASK_FN (rcv_task_upload_media), ctx,
                                 NULL, "upload-media");
    if (!tsk) {
        /* Synchronous reject. Same shape as the chunked entry —
         * no wire write, ctx reclaimed, user_data left to the
         * caller. */
        debug_log ("media", "upload: task_new failed for single-shot; "
                            "synchronous reject");
        ctx->user_data = NULL;
                    ...
                    const guint8 **out_ptr, gsize *out_len)
{
    ...
    gsize remaining = ctx->payload_len > start ? ctx->payload_len - start : 0;
...
send_next_chunk (struct htlc_conn *htlc, hx_upload_ctx *ctx)
{
    ...
    if (ctx->next_part_index == 0 || ctx->next_part_index >= ctx->part_count) {
        debug_log ("media",
                   "upload: chunked continuation index %u out of range "
                   "(part_count=%u)",
                   (unsigned)ctx->next_part_index, (unsigned)ctx->part_count);
        return FALSE;
    }
    ...
    int32_t hc = gtkhx_proto_build_upload_media_followup_chunks (
        ...
        ctx->next_part_index, is_final, chunks, G_N_ELEMENTS (chunks), scratch,
        sizeof (scratch));
    if (hc <= 0) {
        debug_log ("media", "upload: follow-up builder rejected chunk %u/%u",
                   (unsigned)ctx->next_part_index, (unsigned)ctx->part_count);
        return FALSE;
    }
    ...
    debug_log ("media", "→ UPLOAD_MEDIA chunk %u/%u (slice=%zu, final=%d)",
               (unsigned)ctx->next_part_index, (unsigned)ctx->part_count,
               slice_len, (int)is_final);
...
gboolean
hx_send_upload_media_chunked (struct htlc_conn *htlc, const guint8 *payload,
                              gsize payload_len, const char *declared_type,
                              gsize declared_type_len,
                              HxInlineMediaUploadCallback on_done,
                              gpointer user_data, GDestroyNotify user_data_free)
{
    ...
    int32_t hc = gtkhx_proto_build_upload_media_first_chunks (
        first_slice, first_slice_len, (const uint8_t *)declared_type,
        declared_type_len, part_count, chunks, G_N_ELEMENTS (chunks), scratch,
        sizeof (scratch));
    if (hc <= 0) {
        debug_log ("media",
                   "upload: first-chunk builder rejected (slice=%zu, "
                   "declared_type_len=%zu, part_count=%u)",
                   first_slice_len, declared_type_len, (unsigned)part_count);
        return FALSE;
    }
    ...
    if (!tsk) {
        ...
        debug_log ("media", "upload: task_new failed for chunked first chunk; "
                            "synchronous reject");
        ctx->user_data = NULL;
        ctx->user_data_free = NULL;
        upload_ctx_free (ctx);
                      ...
                      GDestroyNotify user_data_free)
{
    ...
    if (payload_len <= (gsize)chunk_size && payload_len <= 65535) {
        return hx_send_upload_media_single (htlc, payload, payload_len,
                                            declared_type, declared_type_len,
                                            on_done, user_data, user_data_free);
    }
    return hx_send_upload_media_chunked (htlc, payload, payload_len,
                                         declared_type, declared_type_len,
                                         on_done, user_data, user_data_free);
}
                 ...
                 hx_upload_ctx *ctx)
{
    ...
    char err_buf[1024];
    gsize err_len = 0;
    if (task_error_extract (frame, frame_len, err_buf, sizeof (err_buf),
                            &err_len)) {
        r.error_message = err_buf;
        r.error_message_len = err_len;
    }

    debug_log ("media", "← UPLOAD_MEDIA error_code=%u (msg='%.*s')",
                 ...
                 hx_upload_ctx *ctx)
{
    ...
    if (!gtkhx_proto_parse_upload_final_reply (frame, frame_len, &parsed)) {
    ...
    debug_log ("media",
               ...
               "bytes=%u",
               r.media_id_len, mime_buf, (unsigned)r.width, (unsigned)r.height,
               (unsigned)r.bytes);
...
void
rcv_task_upload_media (struct htlc_conn *htlc, const guint8 *frame,
                       gsize frame_len, void *ctx_ptr, void *unused)
{
    (void)unused;
    ...
    size_t tok_len = 0;
    if (gtkhx_proto_parse_upload_token_reply (frame, frame_len, &tok_ptr,
                                              &tok_len)
        && tok_len > 0) {
        if (tok_len > HX_MEDIA_MAX_UPLOAD_TOKEN) {
            ...
            deliver_synthetic_failure (htlc, ctx,
                                       "Upload reply token oversized");
            return;
        }
        g_free (ctx->upload_token);
    ...
    if (!ctx->upload_token) {
        ...
        deliver_synthetic_failure (htlc, ctx,
                                   "Upload reply missing session token");
        return;
    }
...
void
hx_send_chat_with_media (struct htlc_conn *htlc, const char *str, guint32 cid,
                         guint16 style, const guint8 *media_id,
                         gsize media_id_len, const char *mime, gsize mime_len)
{
    if (!htlc || !str) {
        return;
    }
    ...
    gboolean have_media
        = (media_id && media_id_len > 0 && media_id_len <= G_MAXUINT16 && mime
    ...
    if (have_media && hc + 2 <= MAX_CHUNKS) {
        ...
        debug_log (
            "media",
            "→ TranChatSend with media (cid=%u handle_len=%zu mime=%.*s)", cid,
            media_id_len, (int)mime_len, mime);
    }
