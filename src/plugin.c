...
static void
unload_plugin (GtkWidget *widget, gpointer data)
{
    char *modname;
...
static void
plugin_row_select (GtkWidget *widget, gint row, gint col, GdkEventButton *event,
                   gpointer data)
{
    plugin_row = row;
    plugin_col = col;
...
static void
close_load_plugin (GtkWidget *btn, GtkWidget *window)
{
    gtkhx_widget_destroy (window);
}

static void
do_load (GtkWidget *m, gpointer n)
{
...
static void
load_plugin (GtkWidget *widget, gpointer data)
{
    GtkWidget *config;
    ...
    gtk_signal_connect (GTK_FILE_SELECTION (config)->ok_button, "clicked",
                        GTK_SIGNAL_FUNC (do_load), config);

    gtk_signal_connect (GTK_FILE_SELECTION (config)->cancel_button, "clicked",
                        GTK_SIGNAL_FUNC (close_load_plugin), config);
...
static void
close_plugin_window (GtkWidget *window, gpointer data)
{
...
create_plugin_manager (void)
{
    ...
    gtk_signal_connect (plugin_window, "delete_event",
                        GTK_SIGNAL_FUNC (close_plugin_window), 0);
    ...
    gtk_signal_connect (unloadbtn, "clicked", GTK_SIGNAL_FUNC (unload_plugin),
                        0);

    closebtn = gtk_button_new_with_label (_ ("Close"));
    gtk_signal_connect (closebtn, "clicked",
                        GTK_SIGNAL_FUNC (close_plugin_window), 0);
    ...
    gtk_clist_set_selection_mode (GTK_CLIST (plugin_list),
                                  GTK_SELECTION_EXTENDED);
    gtk_clist_set_column_width (GTK_CLIST (plugin_list), 0, 40);
    gtk_widget_set_usize (plugin_list, 350, 200);
    gtk_signal_connect (plugin_list, "select_row",
                        GTK_SIGNAL_FUNC (plugin_row_select), 0);
        ...
        while ((ent = readdir (dir))) {
            if (strcmp (ent->d_name, ".") && strcmp (ent->d_name, "..")) {
                if (match (mask, ent->d_name)) {
                    i++;
                    buf = g_malloc (strlen (dirname) + strlen (ent->d_name)
                                    + 2);
