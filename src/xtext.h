 ...
 *
 * -----------------------------------------------------------------------
 * GtkHx Phase 2.6: vendored from HexChat (commit master, 2026-04) with
 * minimal edits — the in-tree XChat 1.8.5 fork has been replaced wholesale
 * by this modern descendant.  Edits relative to upstream:
 *   - gtk_xtext_search_flags inlined here (was in hexchat.h)
 *   - gtk_xtext_set_marker_last(session*) dropped (no consumer in GtkHx)
 *   - gtk_xtext_save() dropped (relied on hexchat write helpers we lack)
...
/*
 * Lastlog/search flags. Inlined here from hexchat.h since GtkHx doesn't
 * vendor the HexChat session model.
 */
