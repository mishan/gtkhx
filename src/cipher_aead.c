...
{
    /* The HKDF info strings are the spec's verbatim ASCII values
	 * (RFC 5869 info parameter). Mismatched info → keys won't
    ...
    uint32_t ref)
{
	 ...
	 *
	 * The ikm concatenates BOTH control-channel keys; we have to
	 * stitch them into a 64-byte buffer before the call. The
	 * order in the spec is encode||decode, no padding. */
    uint8_t ikm_concat[64];
    memcpy (ikm_concat,      ctrl_encode->key, 32);
    memcpy (ikm_concat + 32, ctrl_decode->key, 32);
