...
do_list (HxLocalFilesProvider *self, const char *path)
{
	...
	while ((info = g_file_enumerator_next_file (enumer, NULL, &err))) {
		...
		if (!name) name = g_file_info_get_name (info);
...
{
	if (!self->current_path) return NULL;
	return g_build_filename (self->current_path, name, NULL);
}

gboolean
hx_local_files_provider_mkdir (HxLocalFilesProvider *self,
...
gboolean
hx_local_files_provider_delete (HxLocalFilesProvider *self,
                                const char *name, GError **err)
{
	GFile   *f;
	gboolean ok;
