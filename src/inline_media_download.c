                       ...
                       gboolean with_part_index)
{
    ...
    int32_t hc = gtkhx_proto_build_download_media_chunks (
        ...
        with_part_index ? true : false, chunks, G_N_ELEMENTS (chunks), scratch,
        sizeof (scratch));
    if (hc <= 0) {
        debug_log ("media",
    ...
    struct task *tsk = task_new (htlc, RCV_TASK_FN (rcv_task_download_media),
                                 ctx, NULL, "download-media");
    if (tsk) {
...
inline_media_download_start (struct htlc_conn *htlc, const guint8 *handle,
                             gsize handle_len,
                             HxInlineMediaDownloadCallback on_done,
                             gpointer user_data)
{
    ...
    if (!handle || handle_len == 0 || handle_len > 65535) {
        debug_log ("media", "download: bad handle (len=%zu)", handle_len);
        return NULL;
    }
    ...
    debug_log ("media", "→ DOWNLOAD_MEDIA first request (handle_len=%zu)",
               handle_len);
                 ...
                 hx_inline_media_download *ctx)
{
    ...
    debug_log ("media", "← DOWNLOAD_MEDIA error_code=%u (msg='%.*s')",
...
deliver_success (struct htlc_conn *htlc, hx_inline_media_download *ctx)
{
    ...
    debug_log ("media", "← DOWNLOAD_MEDIA ok (total_bytes=%u, mime=%s)",
...
void
rcv_task_download_media (struct htlc_conn *htlc, const guint8 *frame,
                         gsize frame_len, void *ctx_ptr, void *unused)
{
    ...
    if (!gtkhx_proto_parse_download_reply (frame, frame_len, &parsed)) {
        debug_log ("media", "DOWNLOAD_MEDIA reply missing required fields; "
    ...
    debug_log ("media",
               ...
               (unsigned)ctx->next_part_index, (unsigned)parsed.part_count,
