...
static void on_watcher_signal      (GDBusProxy *p, const char *sender,
                                    ...
                                    gpointer u);
static void on_watcher_owner       (GDBusProxy *p, GParamSpec *pspec,
                                    gpointer u);
                 ...
                 gpointer user_data)
{
	...
	(void) interface; (void) params; (void) user_data;
...
menu_item_properties (int id)
{
	...
	case MENU_ROOT:
		...
		break;
	case MENU_SHOWHIDE:
		label = gtkhx_tray_any_window_visible ()
			? "Hide GtkHx"
			: "Show GtkHx";
		break;
	case MENU_CONNECT:
...
menu_build_layout (int id, int depth)
{
	...
	if (id == MENU_ROOT && depth != 0) {
		int kids[] = { MENU_SHOWHIDE, MENU_CONNECT,
		               MENU_DISCONNECT, MENU_QUIT };
		gsize i;
		...
		for (i = 0; i < G_N_ELEMENTS (kids); i++) {
			g_variant_builder_add_value (
				...
				g_variant_new_variant (
					menu_build_layout (kids[i], sub_depth)));
		}
	}

	return g_variant_new ("(i@a{sv}av)",
	                      id, menu_item_properties (id),
	                      &children);
}
                  ...
                  gpointer user_data)
{
	...
	if (g_strcmp0 (method, "GetLayout") == 0) {
		...
		g_variant_get (params, "(ii^a&s)",
		               &parent_id, &recursion_depth, NULL);
		layout = menu_build_layout (parent_id, recursion_depth);
		g_dbus_method_invocation_return_value (
			invocation,
			g_variant_new ("(u@(ia{sv}av))",
			               tray_menu_revision, layout));
		return;
	}
...
on_watcher_proxy_ready (GObject *src, GAsyncResult *res, gpointer u)
{
	...
	tray_watcher_owner_sigid = g_signal_connect (
		tray_watcher_proxy, "notify::g-name-owner",
		G_CALLBACK (on_watcher_owner), NULL);
	tray_watcher_host_registered_sigid = g_signal_connect (
		tray_watcher_proxy, "g-signal",
		G_CALLBACK (on_watcher_signal), NULL);
...
{
	(void) p; (void) sender; (void) params; (void) u;
...
on_bus_acquired (GDBusConnection *conn, const char *name, gpointer u)
{
	...
	tray_sni_node  = g_dbus_node_info_new_for_xml (
		SNI_INTROSPECTION_XML, NULL);
	tray_menu_node = g_dbus_node_info_new_for_xml (
		DBUSMENU_INTROSPECTION_XML, NULL);

	tray_sni_reg_id = g_dbus_connection_register_object (
		conn, "/StatusNotifierItem",
		tray_sni_node->interfaces[0],
		&sni_vtable, NULL, NULL, &err);
	if (err) {
	...
	tray_menu_reg_id = g_dbus_connection_register_object (
		conn, "/MenuBar",
		tray_menu_node->interfaces[0],
		&menu_vtable, NULL, NULL, &err);
	if (err) {
...
tray_activate_register (void)
{
	...
	tray_bus_name = g_strdup_printf (
		"org.kde.StatusNotifierItem-%d-1", getpid ());
	tray_name_id = g_bus_own_name (
		G_BUS_TYPE_SESSION, tray_bus_name,
...
tray_unregister (void)
{
	if (tray_bus && tray_sni_reg_id) {
		g_dbus_connection_unregister_object (tray_bus,
		                                      tray_sni_reg_id);
		tray_sni_reg_id = 0;
	}
