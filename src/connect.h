...
/* The cipher / compression picker vocabulary (valid_ciphers[],
 * valid_compressors[], valid_cipher/valid_compress, and the two
 * dropdown<->stable-byte translators) moved to cipher_vocab.{c,h}
 * when the Connect dialog moved to Rust (R5.3). Re-included here so
 * historic consumers (bookmarks.c) keep finding them via connect.h. */
