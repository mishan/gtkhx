...
use hxproto::parse::{self, AgreementResult, CatList, DirList, Header, NewsDirEntry, NewsDirKind};
use std::slice;
    ...
    match parse::parse_user_list_record(s, name_cap - 1) {
        Some(rec) => {
            ...
            match rec.nick_color {
                ...
                None => {
                    (*out).nick_color = hxproto::messages::NICK_COLOR_NONE;
...
/// [`hxproto::build::build_htxf_hdr`] for the wire layout (magic + ref +
...
pub unsafe extern "C" fn gtkhx_proto_htxf_hdr_pack(
    ...
    if out.is_null() || out_cap < hxproto::build::HTXF_HDR_SIZE || out_cap > isize::MAX as usize {
        return false;
    }
    let buf = slice::from_raw_parts_mut(out, out_cap);
    hxproto::build::build_htxf_hdr(buf, ref_id, payload_len, type_code, flags)
}
...
///     [`hxproto::build::MAX_PACK_CHUNKS`] entries; we can't materialize
...
unsafe fn hxchunks_to_packchunks<'a>(
    chunks: &'a [hxproto::build::HxChunk],
    out: &'a mut [std::mem::MaybeUninit<hxproto::build::PackChunk<'a>>;
                hxproto::build::MAX_PACK_CHUNKS],
) -> Option<&'a [hxproto::build::PackChunk<'a>]> {
    if chunks.len() > hxproto::build::MAX_PACK_CHUNKS {
        return None;
    }
    for (i, c) in chunks.iter().enumerate() {
        ...
        out[i].write(hxproto::build::PackChunk { tag: c.tag, data });
    }
    ...
    let initialized = std::slice::from_raw_parts(
        out.as_ptr() as *const hxproto::build::PackChunk<'_>,
        chunks.len(),
    );
    Some(initialized)
}
...
pub unsafe extern "C" fn gtkhx_proto_pack_message_size(
    ...
    if chunks_len == 0 {
        ...
        return hxproto::build::pack_message_size(&[]);
    }
    if chunks.is_null()
        || chunks_len > hxproto::build::MAX_PACK_CHUNKS
        || chunks_len > isize::MAX as usize / std::mem::size_of::<hxproto::build::HxChunk>()
    {
        return 0;
    }
    ...
    // anything.
    let mut total = hxproto::HL_HDR_LEN;
    for c in raw {
        total += hxproto::HL_DATA_HDR_LEN + c.len as usize;
    }
    total
}
...
pub unsafe extern "C" fn gtkhx_proto_pack_message(
    ...
    if chunks_len == 0 {
        ...
        // header-only message.
        return hxproto::build::pack_message(buf, type_, trans, flag, &[]).unwrap_or(0);
    }
    if chunks.is_null()
        || chunks_len > hxproto::build::MAX_PACK_CHUNKS
        || chunks_len > isize::MAX as usize / std::mem::size_of::<hxproto::build::HxChunk>()
    ...
    let mut staging: [std::mem::MaybeUninit<hxproto::build::PackChunk<'_>>;
        hxproto::build::MAX_PACK_CHUNKS] =
        [const { std::mem::MaybeUninit::uninit() }; hxproto::build::MAX_PACK_CHUNKS];
    let safe_chunks = match hxchunks_to_packchunks(raw, &mut staging) {
        Some(s) => s,
        None => return 0,
    };
    hxproto::build::pack_message(buf, type_, trans, flag, safe_chunks).unwrap_or(0)
}
...
pub unsafe extern "C" fn gtkhx_proto_pack_header(
    ...
    let out = slice::from_raw_parts_mut(dst, hxproto::HL_HDR_LEN);
    hxproto::build::pack_header(out, type_, trans, flag, hc, body_len);
}
...
pub unsafe extern "C" fn gtkhx_proto_text_to_utf8(
    ...
    hxproto::text::to_utf8_into(s, buf)
}
...
/// alphabetical (see [`hxproto::emoji::shortcode_matches`]).
///
/// # Safety
...
pub unsafe extern "C" fn gtkhx_proto_shortcode_matches(
    ...
    let matches = hxproto::emoji::shortcode_matches(prefix_str, max);
...
pub unsafe extern "C" fn gtkhx_proto_build_voice_join_chunks(
    ...
    let chunks_slice = slice::from_raw_parts_mut(chunks, MAX_CHUNKS);
    let scratch_slice = slice::from_raw_parts_mut(scratch, MAX_SCRATCH);
    hxproto::voice::build_voice_join_chunks(cid, chunks_slice, scratch_slice) as i32
}
...
pub unsafe extern "C" fn gtkhx_proto_build_voice_leave_chunks(
    ...
    let chunks_slice = slice::from_raw_parts_mut(chunks, MAX_CHUNKS);
    let scratch_slice = slice::from_raw_parts_mut(scratch, MAX_SCRATCH);
    hxproto::voice::build_voice_leave_chunks(cid, chunks_slice, scratch_slice) as i32
}
...
pub unsafe extern "C" fn gtkhx_proto_build_voice_answer_chunks(
    ...
    hxproto::voice::build_voice_answer_chunks(cid, sdp, chunks_slice, scratch_slice) as i32
}
...
pub unsafe extern "C" fn gtkhx_proto_build_voice_ice_chunks(
    ...
    hxproto::voice::build_voice_ice_chunks(cid, ice, chunks_slice, scratch_slice) as i32
}
...
pub unsafe extern "C" fn gtkhx_proto_build_voice_mute_chunks(
    ...
    let chunks_slice = slice::from_raw_parts_mut(chunks, MAX_CHUNKS);
    let scratch_slice = slice::from_raw_parts_mut(scratch, MAX_SCRATCH);
    hxproto::voice::build_voice_mute_chunks(cid, muted, chunks_slice, scratch_slice) as i32
}

// ---- Voice parsers ----

/// C-ABI mirror of the Rust [`hxproto::voice::Participant`]. Layout
...
pub unsafe extern "C" fn gtkhx_proto_parse_voice_participants(
    ...
    let mut n = 0;
    for p in hxproto::voice::parse_voice_participants(blob) {
        if n >= cap {
            break;
        }
...
/// `mids` and `bundle` lists from the Rust [`hxproto::voice::sdp::SdpSummary`]
...
pub unsafe extern "C" fn gtkhx_proto_parse_voice_sdp_summary(
    ...
    let summary = hxproto::voice::sdp::summarize(s);
...
pub struct VoiceIceCandidate {
    inner: hxproto::voice::ice::IceCandidate,
}
...
pub unsafe extern "C" fn gtkhx_proto_parse_voice_ice_json(
    ...
    let parsed = match hxproto::voice::ice::parse(s) {
        Some(p) => p,
...
pub unsafe extern "C" fn gtkhx_proto_build_voice_ice_json(
    ...
    let result: Option<hxproto::voice::ice::IceCandidate> = (|| {
        let mut c = hxproto::voice::ice::IceCandidate::default();
    ...
    let json = hxproto::voice::ice::build(&c);
    if json.len() > out_cap {
        return 0;
    }
...
/// [`hxproto::voice::VoiceReply`] but without the borrowed slices —
...
// ===========================================================================
// GIF-icons extension (fogWraith GIF-Icons.md). See hxproto::gif_icons.
// ===========================================================================
...
pub unsafe extern "C" fn gtkhx_proto_gif_icon_is_gif(buf: *const u8, len: usize) -> bool {
    hxproto::gif_icons::is_gif(as_slice(buf, len))
}
...
pub unsafe extern "C" fn gtkhx_proto_build_icon_set_chunks(
    ...
    hxproto::gif_icons::build_icon_set_chunks(gif, chunks_slice) as i32
}
...
pub unsafe extern "C" fn gtkhx_proto_build_icon_get_chunks(
    ...
    let chunks_slice = slice::from_raw_parts_mut(chunks, 1);
    let scratch_slice = slice::from_raw_parts_mut(scratch, MAX_SCRATCH);
    hxproto::gif_icons::build_icon_get_chunks(uid, chunks_slice, scratch_slice) as i32
}

/// C-ABI mirror of [`hxproto::gif_icons::IconEntry`]. `gif_ptr` borrows
...
pub unsafe extern "C" fn gtkhx_proto_parse_icon_get_reply(
    ...
    let s = as_slice(buf, len);
    let Some(e) = hxproto::gif_icons::parse_icon_get_reply(ChunkIter::over_message(s, s.len()))
    else {
        return false;
    };
    *out = IconEntryOut {
...
pub unsafe extern "C" fn gtkhx_proto_parse_icon_change(
    ...
    let s = as_slice(buf, len);
    match hxproto::gif_icons::parse_icon_change(ChunkIter::over_message(s, s.len())) {
        Some(uid) => {
            if !out_uid.is_null() {
                *out_uid = uid;
            }
...
pub unsafe extern "C" fn gtkhx_proto_parse_icon_list(
    ...
    for e in hxproto::gif_icons::parse_icon_list(ChunkIter::over_message(s, s.len())) {
...
/// [`hxproto::hl_date::HlDate`] (mirror: `struct gtkhx_proto_hl_date` in
/// `hotline_proto.h`).
#[repr(C)]
...
pub unsafe extern "C" fn gtkhx_proto_hl_date_decode(
    ...
    match hxproto::hl_date::parse_hl_date(s) {
        Some(hxproto::hl_date::HlDate::Mac1904 { secs }) => {
        ...
        Some(hxproto::hl_date::HlDate::Modern { year, secs }) => {
            *out = GtkhxProtoHlDate {
                kind: 1,
                year,
                secs,
