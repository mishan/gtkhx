...
//! position 0, one tab per private chat (keyed on cid), and one tab per PM
//! conversation (keyed on uid). This module owns the tab view, the two index
//! maps, and the close-page dispatch; `chat.c` / `msg.c` call in to add / find /
//! raise / close tabs through the unchanged `gtkhx_chat_tabs_*` C ABI.
//!
...
struct ChatTabs {
    ...
    pchat: HashMap<u32, adw::TabPage>,
    msg: HashMap<u16, adw::TabPage>,
    on_close_pchat: Option<extern "C" fn(u32)>,
    on_close_msg: Option<extern "C" fn(u16)>,
}
...
fn on_close_page(view: &adw::TabView, page: &adw::TabPage) -> glib::Propagation {
    ...
    if let Some(m) = m {
        STATE.with(|s| {
            ...
            match m.kind {
                KIND_PCHAT => {
                    st.pchat.remove(&m.id);
                    if let Some(f) = st.on_close_pchat {
                        cb = Some(Box::new(move || f(m.id)));
                    }
                }
                KIND_MSG => {
                    st.msg.remove(&(m.id as u16));
                    if let Some(f) = st.on_close_msg {
                        let uid = m.id as u16;
                        cb = Some(Box::new(move || f(uid)));
                    }
                }
                _ => {}
            }
...
#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_set_close_pchat_handler(func: Option<extern "C" fn(u32)>) {
    STATE.with(|s| s.borrow_mut().on_close_pchat = func);
}

#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_set_close_msg_handler(func: Option<extern "C" fn(u16)>) {
...
pub unsafe extern "C" fn gtkhx_chat_tabs_add_pchat(
    ...
    let ptr = page.as_ptr();
    STATE.with(|s| {
        s.borrow_mut().pchat.insert(cid, page);
    });
    ptr
}
...
pub unsafe extern "C" fn gtkhx_chat_tabs_add_msg(
    ...
    let ptr = page.as_ptr();
    STATE.with(|s| {
        s.borrow_mut().msg.insert(uid, page);
    });
    ptr
}

fn find_pchat(cid: u32) -> Option<adw::TabPage> {
    STATE.with(|s| s.borrow().pchat.get(&cid).cloned())
}

fn find_msg(uid: u16) -> Option<adw::TabPage> {
    STATE.with(|s| s.borrow().msg.get(&uid).cloned())
}

#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_find_pchat(cid: u32) -> *mut adw::ffi::AdwTabPage {
    find_pchat(cid)
        .map(|p| p.as_ptr())
...
#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_find_msg(uid: u16) -> *mut adw::ffi::AdwTabPage {
    find_msg(uid)
        .map(|p| p.as_ptr())
        .unwrap_or(std::ptr::null_mut())
}
...
#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_raise_pchat(cid: u32) {
    raise_and_select(find_pchat(cid));
}

#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_raise_msg(uid: u16) {
    raise_and_select(find_msg(uid));
}
...
#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_set_attention_pchat(cid: u32, state: glib::ffi::gboolean) {
    set_page_attention(find_pchat(cid), state != glib::ffi::GFALSE);
}

#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_set_attention_msg(uid: u16, state: glib::ffi::gboolean) {
    set_page_attention(find_msg(uid), state != glib::ffi::GFALSE);
}

/// # Safety
/// `title` is NULL or a valid C string.
#[no_mangle]
pub unsafe extern "C" fn gtkhx_chat_tabs_set_title_pchat(cid: u32, title: *const c_char) {
    if let Some(page) = find_pchat(cid) {
        let t = crate::cstr(title);
        page.set_title(if t.is_empty() { "Private Chat" } else { &t });
    }
}
...
/// `title` is NULL or a valid C string.
#[no_mangle]
pub unsafe extern "C" fn gtkhx_chat_tabs_set_title_msg(uid: u16, title: *const c_char) {
    if let Some(page) = find_msg(uid) {
        let t = crate::cstr(title);
        page.set_title(if t.is_empty() { "PM" } else { &t });
    }
}
...
/// registered teardown.
#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_close_pchat(cid: u32) {
    if let (Some(view), Some(page)) = (view(), find_pchat(cid)) {
        view.close_page(&page);
    }
}

#[no_mangle]
pub extern "C" fn gtkhx_chat_tabs_close_msg(uid: u16) {
    if let (Some(view), Some(page)) = (view(), find_msg(uid)) {
        view.close_page(&page);
    }
}
