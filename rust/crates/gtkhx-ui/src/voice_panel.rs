...
extern "C" {
    ...
    //
    // **Every one of these tolerates NULL**, and that is load-bearing rather
    // than incidental: `PanelInner::sess` resolves a serial and answers NULL
    // once the connection has been closed, so a panel outliving its tab feeds
    // NULL straight through here. The session accessors answer NULL and the
    // htlc predicates answer FALSE, which is the "no connection, no access"
    // reading the panel wants.
    //
    // Do not assume the same of the `hx_conn_*` accessors in `gtkhx-core`:
    // those are generated by the `scalar!` macro and dereference
    // unconditionally, so `hx_conn_fd(NULL)` is undefined behaviour rather
    // than 0. The two families look alike at the call site and are not.
...
fn update_button_labels(inner: &PanelInner) {
    ...
    // No explicit NULL check: `sess()` is NULL once this panel's connection
    // has been closed, and both shims are NULL-tolerant (see the extern
    // block), so a closed connection reads as no-access — which is what
    // should grey the buttons out. The guard that used to be here said the
    // same thing twice.
