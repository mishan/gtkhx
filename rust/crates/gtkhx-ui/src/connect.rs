...
//!
//! Two deliberate boundaries kept in C:
//!   * The cipher / compression picker vocabulary (`valid_ciphers[]` etc.)
//!     lives in `cipher_vocab.c` — shared with the still-C bookmarks.c
//!     dialog. This module reads it through the `cipher_vocab_*` accessors.
...
extern "C" {
    ...
    // cipher_vocab.c — dropdown models + translators.
    fn cipher_vocab_cipher_count() -> c_int;
    fn cipher_vocab_cipher_name(i: c_int) -> *const c_char;
    fn cipher_vocab_compress_count() -> c_int;
    fn cipher_vocab_compress_name(i: c_int) -> *const c_char;
    fn valid_cipher(name: *const c_char) -> c_int;
    fn valid_compress(name: *const c_char) -> c_int;
    fn connect_dropdown_to_cipher_byte(idx: u32) -> u8;
    fn connect_cipher_byte_to_dropdown(byte: u8) -> u32;
    // bookmark_cipher.c — stable byte → HOPE cipher name.
    fn bookmark_cipher_name(byte: u8) -> *const c_char;
    // hotline_url.c — hotline:// URL parser.
    fn hotline_url_parse(url: *const c_char, out: *mut HotlineUrlParts) -> glib::ffi::gboolean;
    // bookmark_rc4_dialog.c — legacy-RC4 replacement picker (sync).
    fn hx_bookmark_rc4_dialog_run_sync(parent: *mut cffi::GtkWindow, name: *const c_char)
        -> c_int;
...
/// Stable bookmark cipher byte for the retired RC4 cipher (bookmark_cipher.h).
const BOOKMARK_CIPHER_BYTE_RC4: u8 = 1;
...
fn connect_with_args(
    ...
    // Resolve the dropdown index / stable byte to HOPE algorithm-name
    // pointers (static C strings, or NULL). Gated on HOPE being on.
    let mut compress_name: *const c_char = std::ptr::null();
    if secure != 0 && compress > 0 {
        unsafe {
            let n = cipher_vocab_compress_name((compress - 1) as c_int);
            if !n.is_null() && valid_compress(n) != 0 {
                compress_name = n;
            }
        }
    }
    ...
    if secure != 0 && cipher != 0 {
        unsafe {
            let n = bookmark_cipher_name(cipher);
            if !n.is_null() && valid_cipher(n) != 0 {
                cipher_name = n;
            }
        }
    }
...
fn set_the_entries_impl(
    ...
    if let Some(c) = &w.compress {
        ...
        let n = unsafe { cipher_vocab_compress_count() } as u32;
        let sel = if (compress as u32) <= n { compress as u32 } else { 0 };
        c.set_selected(sel);
    }
    if let Some(c) = &w.cipher {
        // connect_cipher_byte_to_dropdown already maps unknown bytes to 0.
        c.set_selected(unsafe { connect_cipher_byte_to_dropdown(cipher) });
    }
    if let Some(t) = &w.tls {
        t.set_active(tls != 0);
    }
...
fn server_connect(sess: *mut c_void) {
    ...
    let cipher = unsafe {
        connect_dropdown_to_cipher_byte(w.cipher.as_ref().map(|c| c.selected()).unwrap_or(0))
    };
...
/// Build a NONE-prefixed GtkStringList from a cipher_vocab_* accessor pair.
fn vocab_string_list(count: unsafe extern "C" fn() -> c_int, name: unsafe extern "C" fn(c_int) -> *const c_char) -> gtk::StringList {
    let list = gtk::StringList::new(&["NONE"]);
    let n = unsafe { count() };
    for i in 0..n {
        let p = unsafe { name(i) };
        if !p.is_null() {
            list.append(&unsafe { cstr(p) });
        }
    }
    list
}
...
pub unsafe extern "C" fn create_connect_window(_btn: *mut cffi::GtkWidget, data: *mut c_void) {
    ...
    compress.set_model(Some(&vocab_string_list(
        cipher_vocab_compress_count,
        cipher_vocab_compress_name,
    )));
    ...
    cipher.set_model(Some(&vocab_string_list(
        cipher_vocab_cipher_count,
        cipher_vocab_cipher_name,
    )));
...
fn save_bookmark_from_form(name: &str) {
    ...
    let cipher =
        unsafe { connect_dropdown_to_cipher_byte(w.cipher.as_ref().map(|c| c.selected()).unwrap_or(0)) }
            as i8;
...
fn load_bookmark(name: &str) -> BmLoad {
    ...
    match std::fs::File::open(&path) {
        Ok(f) => {
            if f.take(4).read_to_end(&mut hdr).is_err() {
...
fn rc4_migrate(name: &str, secure: u8, cipher: u8) -> Option<u8> {
    if secure != 0 && cipher == BOOKMARK_CIPHER_BYTE_RC4 {
        ...
        let nb = unsafe { hx_bookmark_rc4_dialog_run_sync(parent, cs(name).as_ptr()) };
        if nb < 0 {
            return None;
        }
...
pub unsafe extern "C" fn connect_bookmark_name(name: *mut c_char) {
    create_connect_window(std::ptr::null_mut(), cffi::hx_active_session());
    open_bookmark_preload(&cstr(name));
}
...
/// which writes to the primary bookmarks dir (the original legacy file is
/// left in place, shadowed by the new copy).
fn convert_bookmark(path: &PathBuf) {
    ...
    unsafe {
        ...
        (*bm).tls = 0;
        let mut err: *mut glib::ffi::GError = std::ptr::null_mut();
        cffi::hx_bookmark_save(bm, &mut err);
        if !err.is_null() {
            glib::ffi::g_error_free(err);
        }
        cffi::hx_bookmark_free(bm);
    ...
    if !existing.is_null() {
        ...
        set_file_error(
            ...
            &format!(
                ...
                tr("Bookmark"),
                name,
                tr("already exists. Manage it from the Bookmarks dialog.")
            ),
        );
        return glib::ffi::GFALSE;
    }
...
/// Parse a port string the way the C dialog's `atoi` did: leading ASCII
/// digits only, everything else (empty, non-numeric, "80x") yields 0 — which
/// makes the connect fail loudly rather than silently falling back to a
/// default. Truncates to 16 bits like the old `int`→`guint16` assignment.
fn atoi_port(s: &str) -> u16 {
    let digits: String = s
        ...
        .collect();
    (digits.parse::<u32>().unwrap_or(0) & 0xffff) as u16
}
