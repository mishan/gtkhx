...
fn connect_pad_added(
    ...
    webrtcbin.connect_pad_added(move |_bin, pad| {
        ...
        main_ctx.invoke(move || {
            with_main_thread_runtime(runtime_id, |rt| {
                // If the synchronous link succeeded, store the bin
                // in receive_bins directly — the state machine's
                // StartReceivePipeline dispatch will then see a
                // pre-existing bin for this mid and short-circuit
                // (the dispatch's early-return covers the
                // "pending_pads empty" case we set up below).
