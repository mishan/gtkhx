...
pub fn encode_alg_list(names: &[&[u8]]) -> Vec<u8> {
    ...
    for n in names {
        // The 1-byte length prefix can't represent >255 bytes.
        // Silently truncating (the old min(255)) would emit a
        // malformed list — count still includes the entry but the
        // payload is short. Every real alg label is a fixed wire
        // constant well under 255, so a longer one is a caller bug:
        // fail loudly. assert! (not debug_assert!) so it holds in
        // release too — see [[feedback_assert_over_debug_assert]].
        assert!(
            n.len() <= 255,
...
pub fn build_step1_login(req: &HopeStep1Request<'_>) -> io::Result<Vec<u8>> {
    ...
    // length would emit an invalid frame; reject it loudly rather
    // than ship malformed bytes. assert! (release too) per
    // [[feedback_assert_over_debug_assert]].
    assert_eq!(app_id.len(), 4, "HOPE app_id must be a 4-byte OSType");
