    ...
    fn verify_server_cert(
        ...
        match self.inner.verify_server_cert(
            ...
            Err(_) => {
                ...
                // mismatch, expired, …). Complete the handshake so the
                // post-handshake TOFU gate can decide; record the miss.
...
fn webpki_client_config() -> (ClientConfig, Arc<AtomicBool>) {
    ...
    let verifier: Arc<dyn ServerCertVerifier> = match inner {
        ...
        None => {
            ...
            Arc::new(AlwaysTofu { webpki_ok: webpki_ok.clone(), provider })
        }
    };
