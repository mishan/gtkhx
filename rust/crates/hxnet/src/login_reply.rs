...
//! - Header signals task failure (`flag != 0`) — returns
//!   `LoginReplyError::TaskRejected` with the error message
//!   from the `DATA_ERROR_TEXT` chunk if present.
//! - Header doesn't decode (oversized wire_len, etc.) —
//!   surfaces as `io::ErrorKind::InvalidData`.
//! - `body_len` exceeds `MAX_BODY_LEN` — surfaces the same way.
    ...
    S: tokio::io::AsyncRead + Unpin,
{
    ...
    if decoded.body_len > MAX_BODY_LEN {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "LOGIN reply body_len {} exceeds MAX_BODY_LEN {}",
                decoded.body_len, MAX_BODY_LEN
            ),
        ));
    }
