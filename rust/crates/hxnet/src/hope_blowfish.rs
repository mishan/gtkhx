...
pub trait HopeRng: Send + 'static {
    /// Fill `buf` with random bytes. These bytes drive only the
    /// *optional* rekey-marker decision (whether this outgoing frame
    /// stamps a rekey marker, and the iteration count if so) — they
    /// are NOT cipher key material. On a randomness failure the
    /// implementation should fall back to a deterministic "no marker"
    /// value (all-zero), which is a safe skip of the rotation rather
    /// than weak crypto.
...
impl HopeRng for OsRng {
    fn fill(&mut self, buf: &mut [u8]) {
        if let Err(e) = getrandom::fill(buf) {
            // A getrandom failure here means the OS entropy source is
            // unavailable. These bytes only decide the *optional*
            // rekey marker (high nibble == {2,7,13} fires), so an
            // all-zero buffer simply means "no marker on this frame" —
            // a safe skip of the rotation. Falling back this way keeps
            // the connection alive (or lets it fail later via the
            // normal io::Error shutdown path) rather than unwinding the
            // actor task / aborting the process under panic=abort.
            glib::g_warning!(
                "hxnet",
                "hope_blowfish: getrandom failed ({e}); skipping the \
                 rekey marker on this frame"
            );
            buf.fill(0);
        }
    }
