...
pub async fn resolve_and_connect(
    ...
    // Resolve. tokio::net::lookup_host accepts "host:port" — we
    // build that here to use its built-in port parsing rather
    // than going through the addrinfo C bindings.
    ...
    if resolved.is_empty() {
        return Err(io::Error::new(
            ...
            format!("lookup_host({target}): empty result"),
        ));
    }
