...
//! Nettle-based implementation in the old cipher.c. The wire protocol
...
impl BlowfishCtx {
    ...
    #[inline]
    fn f(&self, x: u32) -> u32 {
        let a = self.s[0][((x >> 24) & 0xff) as usize];
...
impl BlowfishOfb64State {
    ...
    pub fn crypt(&mut self, src: &[u8], dst: &mut [u8]) {
        ...
        for (s, d) in src.iter().zip(dst.iter_mut()) {
            if n == 0 {
                self.ctx.encrypt_block(&mut self.ivec);
            }
            *d = *s ^ self.ivec[n];
            n = (n + 1) & 7;
        }
...
// Blowfish initial S-boxes (from pi). This is large but necessary for
// a correct Blowfish implementation.
#[rustfmt::skip]
