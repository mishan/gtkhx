 ...
 *       Happy path: hx_connect drives through the magic exchange,
 *       send_login fires HANDSHAKE_DONE, and the bridge is
 *       installed at that point (hx_bridge_is_installed() == TRUE).
 *       The LOGIN frame is popped from htlc->out by the hxnet
 *       routing in hlwrite_chunks — assert htlc->out.len == 0
 *       (the legacy GIOStream path would have left the bytes
 *       buffered).
 *
 *   real_connect_hxnet/uninstall_on_close
 *       After hx_htlc_close runs (driven by the previous test's
 *       teardown), the bridge is uninstalled. The shutdown
 *       callback is the load-bearing path: hxnet's actor exits
 *       when its TcpStream drops, which fires
 *       bridge_on_shutdown_cb on the main loop; that callback
