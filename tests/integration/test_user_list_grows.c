 ...
 */
static gboolean
user_list_contains (int fd, struct htlc_conn *htlc,
	...
	for (int i = 0; i < 64; i++) {
		...
		dh_start (htlc) {
			...
			if (8 + (gsize) nlen > _len)
				continue;
			if (nlen == wlen &&
			    memcmp (dh->data + 8, wanted_name, wlen) == 0) {
				found = TRUE;
				break;
			}
		} dh_end ();
...
main (int argc, char **argv)
{
	...
	g_test_add_func (
		"/integration/user_list/includes_other_client",
