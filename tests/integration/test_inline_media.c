...
encode_test_png (void)
{
    ...
    GError *err = NULL;
    gboolean ok
        = gdk_pixbuf_save_to_buffer (pix, &buf, &bufsz, "png", &err, NULL);
    g_object_unref (pix);
    if (!ok || !buf) {
        g_clear_error (&err);
        return NULL;
                           ...
                           guint32 chunk_size, const char *declared_type)
{
    ...
    if (gtkhx_proto_header_in_error (hx_test_in (htlc)->buf,
                                     hx_test_in (htlc)->pos)) {
        return 0;
    }
    const guint8 *tok_ptr = NULL;
    size_t tok_len = 0;
    if (!gtkhx_proto_parse_upload_token_reply (
            hx_test_in (htlc)->buf, hx_test_in (htlc)->pos, &tok_ptr, &tok_len)
        || tok_len == 0) {
        return 0;
    }
...
send_chat_with_media (int fd, struct htlc_conn *htlc, const char *text,
                      const guint8 *handle, gsize handle_len, const char *mime)
{
    guint16 style = g_htons (1);
    if (handle && handle_len > 0 && mime) {
        return integration_send_message (
            fd, htlc, HTLC_HDR_CHAT, /*flag=*/0, /*hc=*/4, (int)HTLC_DATA_STYLE,
            (int)sizeof (style), &style, (int)HTLC_DATA_CHAT,
            (int)strlen (text), text, (int)HTLC_DATA_CHAT_MEDIA_ID,
            (int)handle_len, handle, (int)HTLC_DATA_CHAT_MEDIA_TYPE,
            (int)strlen (mime), mime);
    }
    return integration_send_message (
...
static guint32
send_download_media (int fd, struct htlc_conn *htlc, const guint8 *handle,
                     gsize handle_len)
{
    guint32 trans_before = htlc->trans;
...
static guint32
send_download_media_part (int fd, struct htlc_conn *htlc, const guint8 *handle,
                          gsize handle_len, guint16 part_index)
{
...
test_inline_media_advisory_limits_advertised (void)
{
     ...
     */
    gboolean any
        = (htlc.media_max_bytes != 0) || (htlc.media_max_dimension != 0)
          || (htlc.media_max_pixels != 0) || (htlc.media_chunk_size != 0)
          || (htlc.media_max_frames != 0) || (htlc.media_max_duration_ms != 0);
    g_assert_true (any);
...
test_inline_media_upload_round_trip (void)
{
    ...
    guint32 trans
        = send_upload_media (fd, &htlc, png_data, png_len, "image/png");
     ...
     * success. */
    g_assert_false (gtkhx_proto_header_in_error (hx_test_in (&htlc)->buf,
                                                 hx_test_in (&htlc)->pos));
...
test_inline_media_chunked_upload_too_large (void)
{
     ...
     * transport hiccup. */
    g_assert_true (gtkhx_proto_header_in_error (hx_test_in (&htlc)->buf,
                                                hx_test_in (&htlc)->pos));
...
test_inline_media_chat_with_media_round_trip (void)
{
    ...
    gboolean chat_sent
        = send_chat_with_media (fd, &htlc, chat_body, handle, handle_len, mime);
    g_free (chat_body);
    ...
    int status = gtkhx_proto_extract_chat_media_meta (
        hx_test_in (&htlc)->buf, hx_test_in (&htlc)->pos, &meta);
...
test_inline_media_download_round_trip (void)
{
    ...
    gboolean is_png = (dl_reply.payload_len >= 8
                       && memcmp (p, "\x89PNG\r\n\x1A\n", 8) == 0);
    gboolean is_jpeg
        = (dl_reply.payload_len >= 3 && memcmp (p, "\xFF\xD8\xFF", 3) == 0);
    gboolean is_gif
        = (dl_reply.payload_len >= 6
...
test_inline_media_chunked_download_round_trip (void)
{
    ...
    guint32 up_trans = send_upload_media_chunked (fd, &htlc, png_data, png_len,
                                                  chunk_size, "image/png");
    g_bytes_unref (png);
    g_assert_cmpuint (up_trans, !=, 0);
    ...
    for (guint16 i = 1; i < part_count && !saw_final; i++) {
        guint32 t = send_download_media_part (fd, &htlc, handle, handle_len, i);
    ...
    gboolean is_png
        = (accumulator->len >= 8 && memcmp (p, "\x89PNG\r\n\x1A\n", 8) == 0);
    gboolean is_jpeg
        = (accumulator->len >= 3 && memcmp (p, "\xFF\xD8\xFF", 3) == 0);
    gboolean is_gif
        = (accumulator->len >= 6
           && (memcmp (p, "GIF87a", 6) == 0 || memcmp (p, "GIF89a", 6) == 0));
    g_assert_true (is_png || is_jpeg || is_gif);
...
test_inline_media_oversized_rejected (void)
{
    ...
    guint32 trans = send_upload_media (fd, &htlc, garbage, garbage_len, NULL);
    g_assert_cmpuint (trans, !=, 0);
    g_assert_true (integration_drain_until_task_trans (fd, &htlc, trans, 16));
...
test_inline_media_unsupported_format_upload (void)
{
    ...
    guint32 trans = send_upload_media (fd, &htlc, (const guint8 *)svg, svg_len,
                                       "image/svg+xml");
    g_assert_cmpuint (trans, !=, 0);
    g_assert_true (integration_drain_until_task_trans (fd, &htlc, trans, 16));
...
test_inline_media_unauthorized_download (void)
{
    ...
    guint32 trans
        = send_download_media (fd, &htlc, fake_handle, sizeof (fake_handle));
    g_assert_cmpuint (trans, !=, 0);
    g_assert_true (integration_drain_until_task_trans (fd, &htlc, trans, 16));
