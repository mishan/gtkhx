...
static gboolean
drain_for_user_change (int fd, struct htlc_conn *htlc, guint16 wanted_uid,
                       int max_messages)
{
...
static gboolean
drain_for_user_part (int fd, struct htlc_conn *htlc, guint16 wanted_uid,
                     int max_messages)
{
    for (int i = 0; i < max_messages; i++) {
        if (!integration_recv_message (fd, htlc, /*timeout_ms=*/3000)) {
...
test_user_change_join_and_part (void)
{
    ...
    g_assert_true (
        drain_for_user_part (fd_a, &htlc_a, htlc_b.uid, /*max_messages=*/64));
