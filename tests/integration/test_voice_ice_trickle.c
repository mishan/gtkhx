...
test_voice_ice_trickle (void)
{
     ...
     * media. */
    const char *cand = "candidate:1 1 UDP 2130706431 127.0.0.1 1 typ host";
    const char *sdp_mid = "0";
    guint8 json_buf[512];
    size_t json_len = gtkhx_proto_build_voice_ice_json (
        (const guint8 *)cand, strlen (cand), (const guint8 *)sdp_mid,
        strlen (sdp_mid),
        /*sdp_mline_index=*/0, /*sdp_mline_index_present=*/true,
        /*username_fragment_ptr=*/NULL, /*username_fragment_len=*/0, json_buf,
        sizeof (json_buf));
