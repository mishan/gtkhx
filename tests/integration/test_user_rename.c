...
static gboolean
drain_for_rename (int fd, struct htlc_conn *htlc, guint16 wanted_uid,
                  const char *wanted_name, struct hx_user_change_msg *out,
                  int max_messages)
{
...
test_user_rename_broadcasts (void)
{
    ...
    int fd_a
        = integration_open_login_or_skip (&htlc_a, "RenameAlice Tier-3", 412);
    if (fd_a < 0) {
        return;
    }
    ...
    int fd_b
        = integration_open_login_or_skip (&htlc_b, "RenameBob Tier-3", 412);
    if (fd_b < 0) {
