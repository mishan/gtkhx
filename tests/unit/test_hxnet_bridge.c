 ...
 *     shaped and will be tested via the harness once R3.3.e-4b
 *     wires the bridge into network.c. Tier 1 here verifies the
 *     translation contract that those callers rely on.
 *
...
struct hxnet_connection_opaque *hxnet_connection_open_plaintext (
    ...
    guint32 trans, test_stub_event_cb on_event,
    ...
    void *user_data)
{
    ...
    (void) name_len; (void) icon; (void) version; (void) trans;
