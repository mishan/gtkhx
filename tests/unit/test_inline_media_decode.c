...
test_sniff_jpeg (void)
{
    ...
    g_assert_cmpstr (inline_media_format_to_mime (INLINE_MEDIA_FORMAT_JPEG), ==,
                     "image/jpeg");
}
...
test_sniff_png (void)
{
    ...
    g_assert_cmpstr (inline_media_format_to_mime (INLINE_MEDIA_FORMAT_PNG), ==,
                     "image/png");
}
...
test_sniff_svg_rejected (void)
{
    ...
    g_assert_cmpint (inline_media_sniff ((const guint8 *)xml, sizeof (xml) - 1),
...
main (int argc, char **argv)
{
    ...
    g_test_add_func ("/inline_media_decode/sniff/unknown", test_sniff_unknown);
    g_test_add_func ("/inline_media_decode/sniff/empty", test_sniff_empty);
