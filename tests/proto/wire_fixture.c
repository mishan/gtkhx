...
void
wire_fixture_init (struct htlc_conn *htlc,
...
void
wire_fixture_add_chunk (struct htlc_conn *htlc,
                        guint16 type, guint16 len, const void *data)
{
	...
	struct hl_data_hdr *dh =
		(struct hl_data_hdr *) (htlc->in.buf + old_pos);
	dh->type = htons (type);
	dh->len  = htons (len);
	if (len && data)
		memcpy (htlc->in.buf + old_pos + SIZEOF_HL_DATA_HDR,
		        data, len);
	...
	guint32 data_len =
		htlc->in.pos - SIZEOF_HL_HDR + sizeof (h->hc);
