...
test_news_file_extracts_body (void)
{
	...
	wire_fixture_add_chunk (&htlc, HTLS_DATA_NEWS,
	                        strlen (body), body);

	char out[256];
	gsize out_len = 0;
	g_assert_true (hx_news_file_extract (&htlc, out, sizeof (out), &out_len));
	g_assert_cmpstr (out, ==,
	                 "Welcome to the news.\n\nRule 1: be nice.");
...
test_news_file_strips_ansi (void)
{
	...
	char out[128];
	g_assert_true   (hx_news_file_extract (&htlc, out, sizeof (out), NULL));
...
test_news_file_no_news_chunk_returns_false (void)
{
	...
	g_assert_false  (hx_news_file_extract (&htlc, out, sizeof (out), NULL));
...
test_news_file_null_out_returns_false (void)
{
	...
	g_assert_false (hx_news_file_extract (&htlc, NULL, 64, NULL));
...
test_news_file_zero_buffer_returns_false (void)
{
	...
	char out[64];
	g_assert_false (hx_news_file_extract (&htlc, out, 0, NULL));
