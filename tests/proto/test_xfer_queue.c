...
test_xfer_queue_extracts_ref_and_queueid (void)
{
	...
	wire_fixture_add_chunk (&htlc, HTLS_DATA_HTXF_REF,
	                        sizeof (ref_wire), &ref_wire);
	wire_fixture_add_chunk (&htlc, HTLS_DATA_QUEUE,
	                        sizeof (queue_wire), &queue_wire);

	struct hx_xfer_queue_msg xq;
	g_assert_true   (hx_xfer_queue_extract (&htlc, &xq));
...
test_xfer_queue_null_out_returns_false (void)
{
	...
	g_assert_false (hx_xfer_queue_extract (&htlc, NULL));
